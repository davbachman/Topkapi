/** Rebuild the curated pilot data; this is a development tool, not app code.
...
const sourceIds = ['4-8-2', '4-6-12', '6'];
const names = ['Rosette · 4.8²', 'Rosette · 4.6.12', 'Rosette · 6³'];
...
function transformedMap(points) {
  assert(
    ...
    'Pilot source polygons must be regular after placement.',
  );
  const center = centroid(points),
    n = points.length;
...
function contactsFor(points, originalEdges) {
  return edgesOf(points).map(({ a, b }) => {
    ...
    assert(
      hits.length <= 1,
      'A pilot edge should cross at most one original edge.',
    );
...
function generate(source, name) {
  ...
  const result = {
    ...
    name,
    description: `Precomputed rosette transform of ${source.name}. Constructed from Craig S. Kaplan’s 2005 “Islamic Star Patterns from Polygons in Contact”, section 4. Contacts preserve intersections with the source tiling${source.id === '4-8-2' ? ', including the off-midpoint adjustment in Figures 11–12' : ''}. Source tiling: Craig S. Kaplan / Taprats and Pierre Baillargeon / Alhambra.`,
    author:
      'Craig S. Kaplan; pilot reconstruction by David Bachman with GPT 6 Astra',
    collection: 'rosette',
...
const tilings = sourceIds.map((id, i) => {
  const source = sourceCatalog.find((t) => t.id === id);
  assert(source, `Missing source tiling ${id}.`);
  return generate(source, names[i]);
});
...
if (process.argv.includes('--check')) {
  assert.equal(
    ...
    output,
    'Precomputed rosette pilot data is stale; rerun this script.',
  );
  console.log('Precomputed pilot data is reproducible.');
} else {
