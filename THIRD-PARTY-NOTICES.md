...
Three additional precomputed rosette-transformed reference tilings were reconstructed from the existing catalog using **Craig S. Kaplan’s** 2005 paper, section 4. Their stored edge contacts implement the correction discussed there, following **Jay Bonner’s** observation that some contacts must move away from edge midpoints. The reproducible generator and pilot data were developed by David Bachman with GPT 6 Astra; the source tilings retain their Taprats/Alhambra attribution. See `docs/ROSETTE-PILOTS.md` for the mathematical benchmarks and scope.
